...
void wait_period( struct timespec * timer_state, unsigned long microseconds )
{
    pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
    pthread_cond_t  cond  = PTHREAD_COND_INITIALIZER;

    /* increment the period */
    timer_state->tv_sec  += microseconds/1000000U;
    timer_state->tv_nsec += (microseconds%1000000U) * 1000;
